...
class Cube:             #큐브 윗면 5번째 앞면 1, 그 오른쪽 2 그 오른쪽 3 그 오른쪽 4 밑면 6 # 또 머하지
    ...
    def __init__(self, mix = None):
        ...
        if mix != None:
            import random
            import time
            for i in range(random.randint(1,30000)):
                self.turn(random.randint(0, 11))
    def check_cube_validity(self):
        """
        큐브 각 면에 있는 색깔 갯수를 체크하고,
        색깔이 9개씩 있는지 확인하는 함수.
        만약 색깔 갯수가 9개가 아니면 신호를 발생시킴.
        """
        is_valid = True
        
        # 큐브의 각 면을 순회하면서 색깔 갯수를 체크
        for face_index, face in enumerate(self.cube):
            cleaned_face = [color for color in face if color.isalpha()]  # 알파벳만 필터링
            
            # 색깔 갯수를 세기 위해 Counter 사용
            color_count = Counter(cleaned_face)
            
            # 색깔이 9개가 아니면 문제가 있음
            for color, count in color_count.items():
                if count != 9:
                    print(f"신호: 면 {face_index + 1}에서 색상 '{color}'의 갯수가 {count}개입니다.")
                    is_valid = False  # 잘못된 색깔 갯수를 발견했으므로 신호 발생
...
"""
    ...
    def vertical_switch_down_right(self):
        ...
        # 각 면의 세로 값들을 mid_list에 수집
        for face in faces:
            for idx in idx_map[face]:
                self.mid_list.append(self.cube[face][idx])

        print("수집된 mid_list:", self.mid_list)  # 디버깅을 위한 출력

        # 오른쪽으로 3칸 회전
        self.mid_list = self.mid_list[-3:] + self.mid_list[:-3]

        #print("회전 후 mid_list:", self.mid_list)  # 디버깅을 위한 출력
        ...
        #print("배치 후 큐브 상태:", self.cube)  # 디버깅을 위한 출력
...
# 오른쪽 메소드 3개
    ...
    def down_switch_right(self):
        self.cube[5] = rotate_90(self.cube[5])  # rotate_90이 정의되어 있다고 가정
        ...
        # mid_list의 마지막 값부터 꺼내서 앞에 삽입하는 과정
        for i in range(3):
            if len(self.mid_list) > 11:  # 리스트 길이 체크
                self.first_value = self.mid_list.pop(11)
                self.mid_list.insert(0, self.first_value)

        # mid_list를 3개씩 끊어서 self.ar에 저장
        self.ar = []
        for i in range(4):
            if len(self.mid_list) > i * 3 + 2:  # 3개씩 끊을 수 있는지 체크
                self.ar.append([self.mid_list[i * 3], self.mid_list[i * 3 + 1], self.mid_list[i * 3 + 2]])

        # cube에 mid_list의 값을 덮어씌우기
        for i in range(4):
            for a in range(3):
                if len(self.mid_list) > i * 3 + a:  # 값이 있을 때만 덮어씌우기
                    self.cube[i][a + 6] = self.mid_list[i * 3 + a]
